...
def compress_img(img_path, rate = 0.5, override = False):
    support_ftype_list = ['png', 'PNG', 'jpeg', 'JPEG', 'gif', 'GIF', 'bmp','jpg']
    ...
    try:
        ...
        print("原始length:{0} height:{1}", length, height)
        print("压缩length:{0} height:{1}", new_length, new_height)
        img = img.resize((new_length, new_height), Image.ANTIALIAS)
        print("resize成功")
        print("保存路径:", compress_img_path)
        print("文件类型:", img_type)
        img.save(compress_img_path, quality=95)
        print("save compress img {0}".format(compress_img_path))
        return True
    except Exception as e:
        print("catch exception 2:{0}",str(e))
...
def thumbnail_pic(github_img_host_base, img_path, cdn_url_prefix):
    ...
    for item in os.listdir(img_path):
        ...
        if os.path.isdir(abs_item): # sub-dir
            ...
            im.save(abs_small_file,  quality=95)  # 保存缩略图
